...
- **Testing note:** the way to test mark-watched/cascade safely is an isolated throwaway Yamtrack (cached images) against a *copy* of the DB — never the live instance. Query logic is unit-testable against a crafted fixture DB with no Yamtrack at all. (21 cases passed this way: gaps, aired/status filters, badges, ordering, empty state, season+show auto-completion, duplicate marks, session-expiry retry, error handling.)
